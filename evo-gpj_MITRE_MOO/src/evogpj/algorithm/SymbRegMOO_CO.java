...
import interpreter.misc.DataFile;
...
public class SymbRegMOO_CO {
    ...
    public SymbRegMOO_CO(Properties props,long timeout) throws IOException {
        this();
        String fname = "C:\\Users\\Jacob\\Documents\\MIT\\SCOTE\\MITRE_coevolution\\Tax\\Tax\\src\\interpreter\\dataFile.csv";
        dataFile = new DataFile(fname);
        if (timeout > 0)
            TIMEOUT = startTime + (timeout * 1000);
        loadParams(props);
//        SEED = Long.parseLong("1395064538490");
...
//    	also get all of the mean and sd and stuff
    	ArrayList<Double> allSchemeFitnesses = new ArrayList<Double>();
    	ArrayList<Double> topSchemeFitnesses = new ArrayList<Double>();
    	for (int i=0 ; i<schemes.size() ; ++i ) {
    		schemes.get(i).setDominationCount(i);
    		allSchemeFitnesses.add(schemes.get(i).getFitness());
    		if (i <= 9) {
    			topSchemeFitnesses.add(schemes.get(i).getFitness());
    		}
    	}
    	
    	ArrayList<Double> allCodeFitnesses = new ArrayList<Double>();
    	ArrayList<Double> topCodeFitnesses = new ArrayList<Double>();
    	for (int i=0 ; i<clauses.size() ; ++i ) {
    		clauses.get(i).setDominationCount(i);
    		allCodeFitnesses.add(clauses.get(i).getFitness());
    		if (i <= 9) {
    			topCodeFitnesses.add(clauses.get(i).getFitness());
    		}
    	}
    	
    	dataFile.getMeanAllSchemes().add(this.mean(allSchemeFitnesses));
    	dataFile.getSdAllSchemes().add(this.sd(allSchemeFitnesses));
    	dataFile.getMeanTopSchemes().add(this.mean(topSchemeFitnesses));
    	dataFile.getSdTopSchemes().add(this.sd(topSchemeFitnesses));
    	dataFile.getBestScheme().add(schemes.get(0).getFitness());
    	
    	dataFile.getMeanAllCodes().add(this.mean(allCodeFitnesses));
    	dataFile.getSdAllCodes().add(this.sd(allCodeFitnesses));
    	dataFile.getMeanTopCodes().add(this.mean(topCodeFitnesses));
    	dataFile.getSdTopCodes().add(this.sd(topCodeFitnesses));
    	dataFile.getBestCode().add(clauses.get(0).getFitness());
    	
    }
    
    
    public double mean(ArrayList<Double> aList) {
    	if (aList.isEmpty())
    		return 0;
    	double total = 0.0;
    	for (double num : aList)
    		total += num;
    	
    	return total / aList.size();
    }
    
    public double sd(ArrayList<Double> aList) {
    	if (aList.isEmpty())
    		return 0;
    	double total = 0.0;
    	double mean = this.mean(aList);
    	
    	ArrayList<Double> dist = new ArrayList<Double>();
    	for (double num : aList) {
    		dist.add(Math.pow(mean - num, 2.0));
    	}
    	
    	
    	double ret = Math.sqrt(this.mean(dist));
...
//    	also get all of the mean and sd and stuff
    	ArrayList<Double> allSchemeFitnesses = new ArrayList<Double>();
    	ArrayList<Double> topSchemeFitnesses = new ArrayList<Double>();
    	for (int i=0 ; i<pops.get("TaxFitness").size() ; ++i ) {
    		allSchemeFitnesses.add(pops.get("TaxFitness").get(i).getFitness());
    		if (i <= 9) {
    			topSchemeFitnesses.add(pops.get("TaxFitness").get(i).getFitness());
    		}
    	}
    	
    	ArrayList<Double> allCodeFitnesses = new ArrayList<Double>();
    	ArrayList<Double> topCodeFitnesses = new ArrayList<Double>();
    	for (int i=0 ; i<pops.get("TaxCodeFitness").size() ; ++i ) {
    		allCodeFitnesses.add(pops.get("TaxCodeFitness").get(i).getFitness());
    		if (i <= 9) {
    			topCodeFitnesses.add(pops.get("TaxCodeFitness").get(i).getFitness());
    		}
    	}
    	
    	Individual scheme = bestOnes.get("TaxFitness");
    	Individual code = bestOnes.get("TaxCodeFitness");
    	
    	dataFile.getMeanAllSchemes().add(this.mean(allSchemeFitnesses));
    	dataFile.getSdAllSchemes().add(this.sd(allSchemeFitnesses));
    	dataFile.getMeanTopSchemes().add(this.mean(topSchemeFitnesses));
    	dataFile.getSdTopSchemes().add(this.sd(topSchemeFitnesses));
    	dataFile.getBestScheme().add(scheme.getFitness());
    	
    	dataFile.getMeanAllCodes().add(this.mean(allCodeFitnesses));
    	dataFile.getSdAllCodes().add(this.sd(allCodeFitnesses));
    	dataFile.getMeanTopCodes().add(this.mean(topCodeFitnesses));
    	dataFile.getSdTopCodes().add(this.sd(topCodeFitnesses));
    	dataFile.getBestCode().add(code.getFitness());
    	
    	

    	System.out.println(scheme.getPhenotype().getPhenotype());
    	System.out.println(scheme.getFitness());
    	System.out.println(code.getPhenotype().getPhenotype());
    	System.out.println(code.getFitness());
    }
...
//		VM output, with time stamp
		...
		try {
			...
			if (bash)
				dataFile.setFname("dataFile_"+t_stamp+".csv");
